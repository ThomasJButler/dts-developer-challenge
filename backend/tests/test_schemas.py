...
class TestTaskCreate:
    """Validation rules for the POST /tasks request body."""

    def test_requires_title(self):
        """Empty string or missing title is rejected."""
    ...
    def test_rejects_naive_due_at(self):
        """Datetimes without a timezone offset are rejected."""
    ...
    def test_accepts_null_description(self):
        """`description` is optional; absent or `None` both produce `None`."""
...
class TestTaskUpdateStatus:
    """Validation rules for the PATCH /tasks/{id}/status request body."""
    ...
    def test_accepts_each_valid_status(self):
        """Every member of `TaskStatus` round-trips through the schema."""
        for status in TaskStatus:
            assert TaskUpdateStatus(status=status).status == status


class TestTaskRead:
    """Serialisation rules for the response body shared by every task endpoint."""

    def test_constructs_from_orm_like_object(self):
        """`model_validate` reads attributes from any object that exposes them."""
