...
5. `docs/design-handoff/README.md` - the handoff overview: stack, routes, task model, what's in and out of scope for the UI.
6. `docs/design-handoff/spec.html` - the primary build reference. Annotated Nunjucks per screen, rationale per choice, full UK English microcopy, accessibility checklist, out-of-scope list.
7. `docs/design-handoff/screenshots/*.png` - the seven screen states. Read each PNG.
