...
- [ ] `Task` SQLAlchemy model using the canonical fields: `id` (uuid), `title`, `description`, `status`, `due_at` (UTC), `created_at`, `updated_at`.
- [ ] Alembic autogenerated migration. Eyeball the SQL before committing.
- [ ] Pydantic schemas: `TaskCreate`, `TaskUpdateStatus`, `TaskRead`.
- [ ] `TaskStatus` enum (`todo`, `in_progress`, `done`) shared between model and schema.
